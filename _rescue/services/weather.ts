...
function getApiKey(): string {
  if (!config?.apiKey) {
    ...
    const envApiKey = process.env.EXPO_PUBLIC_OPENWEATHERMAP_API_KEY;
    if (envApiKey) {
      return envApiKey;
    }
    throw new Error('OpenWeatherMap API key not configured. Call setWeatherApiKey() first.');
  }
...
export async function fetchHistoricalWeather(
  ...
  if (hoursDiff < 3) {
    console.log('[Weather] Catch is recent, using current weather instead of historical');
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      console.warn('[Weather] One Call 3.0 requires paid subscription. Skipping historical fetch.');
      // For older catches without paid API, we can't get historical data
      // Return a placeholder that indicates weather couldn't be fetched
      throw new Error('Historical weather requires One Call 3.0 subscription (paid)');
    }
    
    const errorText = await response.text();
    console.error('[Weather] Historical API error:', response.status, errorText);
...
export async function fetchWeather(
  ...
  try {
    ...
    console.warn('[Weather] Historical fetch failed, using current weather as fallback');
    return fetchCurrentWeather(latitude, longitude);
  }
}
